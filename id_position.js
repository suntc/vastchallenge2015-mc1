...
function Bar(bar) {
	var grid = d3.select(this).selectAll(".grid")
				.data(function(d) { return removeMovingPath(d.path); })
				.enter()
				.append("rect")
				.attr("class", "grid")
				...
				.on("mouseout", function(d, i) {
					...
					if (d != -1) removeElement(d);
				});
}

function removeMovingPath(path) {
	// Change zero id sites to its last visited site
